...
function render_list(user_id) {
    ...
    count_element.innerText = `(${user_id.length} IDs)`;

    let user_id_list = document.getElementById("id-list");
    user_id_list.innerHTML = user_id.map(user => `
...
function renderSessionStream(session_id, user) {
    ...
    let total_lenght_x = session_time_max * 1.05;
    ...
    let context_lables = {0: "Word Cloud Term", 1: "TopicCloud Term", 2: "History Term", 3: "Chapter Expanded",
    ...
    let common = {domain: [0, total_lenght_x], grid: true};

    //BARPLOT
    let plot_histo = Plot.plot({
        width: container.clientWidth, height: 250, marginLeft: 70, marginBottom: 5, marginRight: 20,
        x: {... common, ticks: [], label: null, axis: null},
        y: {label: "Event Intensity", grid: true, nice: true, ticks: 5},
        color: {domain: types, range: colors, legend: true, tickFormat: (_, label) => legend_labels[label]},
        marks: [
        Plot.text(["Mouse\nand\nKeypress\nEvents"], {
            frameAnchor: "left",  dx: -47, fontSize: 10, textAnchor: "middle", lineHeight: 1.2
            }),
            ...types.map((type, index) => 
                Plot.rectY(events.filter(data => data.type === type), 
                    Plot.binX({y: "count",
                         title: (bin) => { 
                            ...
                            if (components[0] === "Keyboard") {
                                return text = `${legend_labels[index]}\nEvents: ${count}\nString: ${string_input}\nInput: ${keypress.join(", ")}`;
                                }
                            return text = `${legend_labels[index]}\nEvents: ${count}\nComponent(s): ${components.join(", ")}`
                            }}, 
                        {x: "time", fill: colors[index], interval: bin_interval,
    ...
    let plot_timeline_dwelling = Plot.plot({
        width: container.clientWidth, height: 20, marginLeft: 70, marginTop: 0, marginBottom: 0, marginRight: 20,
        x: {... common, label: null, axis: null, tickFormat: () => ""},
        y: {domain: [0, 1], axis: null},
        marks: [
            Plot.text(["Dewelling"], {
            frameAnchor: "left", dx: -47, fontSize: 10, textAnchor: "middle"
            }),
            Plot.rectX(session_dwellings, 
                ...
                Plot.text(session_dwellings, {x: dwell => (dwell.start + dwell.end) / 2, y: 0.5, 
                        text: dwell => {
                            ...
                            let available_width = ((container.clientWidth - 90) / total_lenght_x) * dwell_duration;
    ...
    let plot_timeline_hovering = Plot.plot({
        width: container.clientWidth, height: 50, marginLeft: 70, marginTop: 0, marginBottom: 30, marginRight: 20,
            ...
            Plot.rectX(session_hoverings, 
                ...
                Plot.text(session_hoverings, {x: hover => (hover.start + hover.end) / 2, y: 0.5, 
                        text: hover => {
                            ...
                            let available_width = ((container.clientWidth - 90) / total_lenght_x) * hover_duration;
                            let max_chars = Math.floor(available_width / 6);
    ...
    container.appendChild(plot_histo);
